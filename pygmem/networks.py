...
class LinearRandomEffects(torch.nn.Module):
    ...
    Arg : 
        n_group (int) : number of individual coefficient to learn
    '''
...
class LinearMixedEffects_fast(torch.nn.Module):
    ...
    Indeed, we can put the parameters individually into different pytorch optimizers,
    and then update the coefficient faster, thanks to the ModuleDict (Dictionary
    of linear models). There is in this model, the fixed effects AND the random effects.
    Arg : 
        n_group (int) : number of individual coefficient to learn
    '''
...
class MemeNet(torch.nn.Module):
    ...
    def __init__(self, n_X, n_Z, n_group, enc_in=[1, 4], enc_out=[2, 30, 1]):
        ...
        self.fc_in = [torch.nn.Linear(enc_in[i], enc_in[i+1]) for i in range(len(enc_in)-1)]
        self.linear_mixed_effects = LinearMixedEffects_fast(enc_in[-1], enc_in[-1], n_group, n_y=enc_out[0])
        self.fc_out = [torch.nn.Linear(enc_out[i], enc_out[i+1]) for i in range(len(enc_out)-1)]
        # print(self.fc_in, self.fc_out)
        # self.fc2 = torch.nn.Linear(2, 30)
        # self.fc3 = torch.nn.Linear(30, n_y)
        self.linear_f = self.linear_mixed_effects.linear_f
        self.linears = self.linear_mixed_effects.linears
...
#     '''Linear Mixed Effect Model, but suited for coordinated gradient descent \
#     and with matrix factorization terms. There is in this model, the fixed effects
